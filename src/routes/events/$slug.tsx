...
function EventDetail() {
  ...
  return (
    <PageFade className="pt-24 pb-24">
      {/* Hero */}
      ...
      <section className="mx-auto max-w-6xl px-5 -mt-6">
        <div className="grid grid-cols-3 gap-3 sm:gap-5">
          {[
            { l: e.slug === "drone-racing" ? "Course Size" : e.slug === "line-follower" ? "Track Length" : "Arena", v: e.arena, i: MapPin },
            { l: e.slug === "drone-racing" ? "Gates / Hoops" : e.slug === "line-follower" ? "Line Width" : "Rounds", v: e.rounds, i: Cpu },
            { l: e.slug === "drone-racing" ? "Race Window" : e.slug === "line-follower" ? "Time Limit" : "Time", v: e.time, i: Timer },
          ].map((s, i) => (
            <Reveal key={s.l} delay={i * 0.08} className="glass rounded-2xl p-5 text-center" style={{ boxShadow: `inset 0 0 0 1px ${c}33` }}>
      ...
      <section className="mx-auto max-w-6xl px-5 mt-12 grid lg:grid-cols-3 gap-5">
        <Card title="The Challenge" color={c} className="lg:col-span-2" delay={0}>
          <p className="text-white/75 leading-relaxed">{e.description}</p>
        </Card>

        <Card title={e.slug === "innovation" ? "Project Specs" : "Robot Specs"} color={c} delay={0.05}>
        ...
        <Card title={e.slug === "innovation" ? "Project Rules" : "Game Rules"} color={c} className="lg:col-span-2" delay={0.1}>
        ...
        <Card title="Your Squad" color={c} delay={0.15}>
        ...
        <Card title="How to Win 🏆" color={c} className="lg:col-span-3" delay={0.2}>
          <ul className="grid sm:grid-cols-2 gap-3">
            {e.winning.map((w, i) => (
              <motion.li
                ...
                className="flex items-start gap-2 rounded-xl p-3 border border-white/10 bg-white/[0.02]"
              >
      ...
      <section className="mx-auto max-w-6xl px-5 mt-14">
        <Reveal className="relative rounded-3xl overflow-hidden glass p-8 sm:p-12 text-center" style={{ boxShadow: `0 0 80px -20px ${c}88, inset 0 0 0 1px ${c}55` }}>
          ...
          <div className="relative">
            ...
            <Link to="/register" className="mt-7 inline-flex items-center gap-2 rounded-full px-7 py-3.5 font-bold text-black hover:scale-105 transition-transform" style={{ background: c, boxShadow: `0 0 40px ${c}77` }}>
...
function Card({ title, color, children, className = "", delay = 0 }: { title: string; color: string; children: React.ReactNode; className?: string; delay?: number }) {
  return (
    <Reveal delay={delay} className={`relative rounded-2xl glass p-6 ${className}`}>
