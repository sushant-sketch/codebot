      ...
      {EVENTS.map((e, i) => (
        ...
        >
          <Link
            to="/events/$slug"
            params={{ slug: e.slug }}
            className="group relative block rounded-2xl glass glass-hover overflow-hidden p-6 h-full"
            style={{ "--c": e.color } as React.CSSProperties}
          >
            <div
              className="absolute -top-20 -right-20 h-48 w-48 rounded-full blur-3xl opacity-30 group-hover:opacity-60 transition-opacity duration-500"
              style={{ background: e.color }}
            />
            <div className="relative flex items-start justify-between">
              <span className="text-[10px] tracking-[0.3em] text-white/40">EVENT · {e.no}</span>
              <ArrowUpRight className="h-4 w-4 text-white/40 group-hover:text-white group-hover:rotate-45 transition duration-300" />
            </div>
            <div className="relative mt-6 flex items-center gap-4">
              <div
                className="h-14 w-14 rounded-xl grid place-items-center text-3xl group-hover:scale-110 transition-transform duration-300"
            ...
            <div className="relative mt-6 flex items-center justify-between text-xs">
              ...
              <span className="font-semibold group-hover:translate-x-1 transition-transform duration-300 inline-block" style={{ color: e.color }}>View Event →</span>
            </div>
          </Link>
        </motion.div>
      ))}
